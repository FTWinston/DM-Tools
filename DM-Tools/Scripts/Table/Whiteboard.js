...
function DrawIt(drawObject, syncServer) {
    ...
    if (syncServer && drawObject.Tool != DrawTool.Pencil) {
        ...
        table.server.drawWhiteboard(tableID, message);
    }
}
...
function initWhiteboard() {
    table.client.handleDraw = function (message, name) {
        
        if (currentDrawingUser != name) {
            currentDrawingUser = name;
        ...
        for (var i = 0; i < drawObjectCollection.length; i++) {
            ...
            if (drawObjectCollection[i].currentState && drawObjectCollection[i].currentState == DrawState.Completed) {
                currentDrawingUser = null;
                $("#divStatus").html("");
            }
        }
    };
